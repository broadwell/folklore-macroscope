...
//  $keywordsQuery = 'SELECT DISTINCT lemmatized_keywords.keyword_id, lemmatized_keywords.keyword FROM lemmatized_keywords, story_to_place_mentioned, story_to_lemmatized_keyword WHERE lemmatized_keywords.keyword_id=story_to_lemmatized_keyword.keyword_id AND story_to_lemmatized_keyword.story_id=story_to_place_mentioned.story_id AND story_to_place_mentioned.place_id=' .  $placeID;
  $keywordsQuery = 'SELECT DISTINCT etk_index_merged.etk_index_id, etk_index_merged.heading_danish FROM etk_index_merged, story_to_place_mentioned, story_to_mimno_etk_index WHERE etk_index_merged.etk_index_id=story_to_mimno_etk_index.etk_index_id AND story_to_mimno_etk_index.story_id=story_to_place_mentioned.story_id AND story_to_place_mentioned.place_id=' .  $placeID;
  ...
  while ($keywordsRow = mysql_fetch_assoc($keywordsResult)) {
    ...
    if (!isset($inKeywords[$keywordsRow['etk_index_id']])) {
      $inKeywords[$keywordsRow['etk_index_id']] = 1;
      $keywordIDtoKeyword[$keywordsRow['etk_index_id']] = $keywordsRow['heading_danish'];
      $regionalCooccurrences++;
    } else {
      $inKeywords[$keywordsRow['etk_index_id']]++;
      $regionalCooccurrences++;
    }
...
foreach ($inKeywords as $keywordID => $total) {
  ...
  $globalKeywordQuery = "SELECT place_cooccurrences FROM etk_index_merged WHERE etk_index_id=".$keywordID;
