...
var helpText = "<p>Selecting a story category from the list at left will produce map visualizations of the " +
               ...
               "<p>Alternately, dragging from an empty point on the map to create a bounding box will generate ranked " +
               "lists of the story keywords that co-occur with the names of places in that region.</p>";
...
//  var storyURL = "http://localhost/~broadwell/maps/witchhunter/GetStories.php?stories=" + uniqueStoriesString; 
  ...
  $.getJSON( storyURL, function(d) {
    var storyDetails = '<h2 class="sub-header">Story texts for most recently selected marker</h2>';
  ...
  var blurbtext = "<p>" + freq + storiesWord + 'in the category "' + indexCategories[f.getAttribute("index")] + '" collected at ' + f.getAttribute("sourceCoords") + mentionWord + ' place(s) at ' + f.getAttribute("destCoords") + "<br>Bearing: " + f.getAttribute("bearing") + "&deg;<br>Distance: " + f.getAttribute("distance") + " km" + "<br><b>Story: Collection=>Mentioned</b>" + refInfo + "</p>";
...
//  var storyURL = "http://localhost/~broadwell/maps/witchhunter/GetStories.php?stories=" + uniqueStoriesString; 
  ...
  var blurbtext = "<p>" + freq + storiesWord + 'in the category "' + indexCategories[f.getAttribute("index")] + '"' + mentionWord + 'this place. See below the map for their texts and metadata.</p><p><b>Place ccoordinates</b><br>' + f.getAttribute("lat") + "N, " + f.getAttribute("lon") + "E</p><p><b>Story ID => Place name (place ID)</b><br>" + f.getAttribute("places") + "</p>";
  //var blurb = "<div class='info_blurb'>" + blurbtext + "</div>";
